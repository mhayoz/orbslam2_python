...
public:
    ...
    bool isRunning();
    bool mapChanged();
    bool loadAndProcessMono(std::string imageFile, std::string segMaskFile, double timestamp);
    bool processMono(cv::Mat imag, cv::Mat segMaske, double timestamp);
    bool loadAndProcessStereo(std::string leftImageFile, std::string rightImageFile, std::string segMaskFile, double timestamp);
    bool processStereo(cv::Mat leftImage, cv::Mat rightImage, cv::Mat segMask, double timestamp);
    bool loadAndProcessRGBD(std::string imageFile, std::string depthImageFile, std::string segMaskFile, double timestamp);
    bool processRGBD(cv::Mat image, cv::Mat depthImage, cv::Mat segMask, double timestamp);
    void reset();
    void shutdown();
    ...
    boost::python::tuple getPose() const;
