...
BOOST_PYTHON_MODULE(orbslam2)
{
    ...
    boost::python::class_<ORBSlamPython, boost::noncopyable>("System", boost::python::init<const char*, const char*, boost::python::optional<ORB_SLAM2::System::eSensor>>())
        ...
        .def("map_changed", &ORBSlamPython::mapChanged)
        ...
        .def("get_residual_error", &ORBSlamPython::getResidualError)
...
bool ORBSlamPython::loadAndProcessMono(std::string imageFile, std::string segMaskFile, double timestamp)
{
    if (!system)
    {
        return false;
    ...
    cv::Mat segMask = cv::imread(segMaskFile, cv::IMREAD_GRAYSCALE);
    if (bUseRGB)
    {
        cv::cvtColor(im, im, cv::COLOR_BGR2RGB);
    }
    return this->processMono(im, segMask, timestamp);
}

bool ORBSlamPython::processMono(cv::Mat image, cv::Mat segMask, double timestamp)
{
    if (!system)
    {
        return false;
    ...
    if (image.data)
    {
        cv::Mat pose = system->TrackMonocular(image, segMask, timestamp);
        return !pose.empty();
    }
    else
    {
...
bool ORBSlamPython::loadAndProcessStereo(std::string leftImageFile, std::string rightImageFile, std::string segMaskFile, double timestamp)
{
    if (!system)
    {
        return false;
    ...
    return this->processStereo(leftImage, rightImage, segMask, timestamp);
}

bool ORBSlamPython::processStereo(cv::Mat leftImage, cv::Mat rightImage, cv::Mat segMask, double timestamp)
{
    if (!system)
    {
        return false;
    ...
    if (leftImage.data && rightImage.data) {
        cv::Mat pose = system->TrackStereo(leftImage, rightImage, segMask, timestamp);
        return !pose.empty();
    }
    else
    {
...
bool ORBSlamPython::mapChanged()
{
    if (!system)
    {
        return false;
    }
    return system->MapSlightlyChanged();
}


bool ORBSlamPython::loadAndProcessRGBD(std::string imageFile, std::string depthImageFile, std::string segMaskFile, double timestamp)
{
    if (!system)
    {
        return false;
    ...
    return this->processRGBD(im, imDepth, segMask, timestamp);
}

bool ORBSlamPython::processRGBD(cv::Mat image, cv::Mat depthImage, cv::Mat segMask, double timestamp)
{
    if (!system)
    {
        return false;
    ...
    {
        cv::Mat pose = system->TrackRGBD(image, depthImage, segMask, timestamp);
        return !pose.empty();
    }
    else
    {
...
boost::python::list ORBSlamPython::getAllMappoints() const
{
    ...
    // This is copied from the ORB_SLAM2 System.SaveTrajectoryKITTI function, with some changes to output a python tuple.
    vector<ORB_SLAM2::MapPoint*> Mps = system->GetAllMapPoints();
    boost::python::list map_points;
    for(size_t i=0; i<Mps.size(); i++)    {
       if(Mps[i] != NULL){
            cv::Mat wp = Mps[i]->GetWorldPos();
...
boost::python::tuple ORBSlamPython::getPose() const
{
    if (!system)
    {
    ...
    boost::python::tuple pose;

    // Frame pose is stored relative to its reference keyframe (which is optimized by BA and pose graph).
    // We need to get first the keyframe pose and then concatenate the relative transformation.
    // Frames not localized (tracking failure) are not saved.

    ORB_SLAM2::KeyFrame* pKF = system->GetTracker()->mlpReferences.back();
    cv::Mat lit=system->GetTracker()->mlRelativeFramePoses.back();
...
float ORBSlamPython::getResidualError()
{
	return system->GetChi2Error();
}
// ----------- HELPER DEFINITIONS -----------
